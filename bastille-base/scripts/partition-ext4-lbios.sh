...
echo ">>>> partition-ext4-lbios.sh: Writing Filesystem types.."
mkfs.ext4 -L BOHKS_EQAHM ${ROOT_PARTITION}
mkfs.ext4 ${BOOT_PARTITION}

echo ">>>> partition-ext4-lbios.sh: Mounting partitions.."
/usr/bin/mount ${ROOT_PARTITION} ${ROOT_DIR}
/usr/bin/mkdir -p ${BOOT_DIR}
/usr/bin/mount ${BOOT_PARTITION} ${BOOT_DIR}

echo ">>>> partition-ext4-lbios.sh: Bootstrapping the base installation.."
...
echo ">>>> partition-ext4-lbios.sh: Installing databases.."
...
echo ">>>> partition-ext4-lbios.sh: Installing basic packages.."
/usr/bin/arch-chroot ${ROOT_DIR} pacman -S --noconfirm sudo gptfdisk openssh grub dhcpcd netctl

echo ">>>> partition-ext4-lbios.sh: Generating the filesystem table.."
/usr/bin/genfstab -U ${ROOT_DIR} | tee -a "${ROOT_DIR}/etc/fstab"

echo ">>>> partition-ext4-lbios.sh: Configuring grub.."
...
echo ">>>> partition-ext4-lbios.sh: Generating the system configuration script.."
