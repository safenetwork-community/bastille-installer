...
echo ">>>> partition-btrfs-efi.sh: Mounting partitions.."
/usr/bin/mount ${ROOT_PARTITION} ${ROOT_DIR}
/usr/bin/mkdir -p ${BOOT_DIR}
/usr/bin/mount ${BOOT_PARTITION} ${BOOT_DIR}

echo ">>>> partition-btrfs-efi.sh: Bootstrapping the base installation.."
/usr/bin/pacstrap ${ROOT_DIR} base btrfs-progs edk2-ovmf `pacman -Qq linux`

echo ">>>> partition-btrfs-efi.sh: Updating pacman mirrors base installation.."
...
echo ">>>> partition-btrfs-efi.sh: Installing databases.."
...
echo ">>>> partition-btrfs-efi.sh: Installing basic packages.."
/usr/bin/arch-chroot ${ROOT_DIR} pacman -S --noconfirm sudo efibootmgr gptfdisk openssh grub dhcpcd netctl

echo ">>>> partition-btrfs-efi.sh: Generating the filesystem table.."
